...
function buildGoogleCalendarUrl({
  ...
  const params = new URLSearchParams({
    action: "TEMPLATE",
    text: title,
    dates: `${formatForGoogleCalendar(
      start
    )}/${formatForGoogleCalendar(end)}`,
    details,
    location,
  });
...
export default function MySchedulePage() {
  ...
  useEffect(() => {
    ...
    async function loadPage() {
      try {
        ...
        /*
         * ---------------------------------------------------------
         * 2. FIRST TRY THE PERMANENT USER_ID LINK
         * ---------------------------------------------------------
         */
        ...
        /*
         * ---------------------------------------------------------
         * 3. AUTO-LINK BY EMAIL IF NOT YET LINKED
         * ---------------------------------------------------------
         */
        ...
        if (!volunteerData && user.email) {
          ...
          if (emailVolunteer) {
            /*
             * Never overwrite a volunteer already linked
            ...
            if (!emailVolunteer.user_id) {
              ...
              /*
               * If another request linked the record between
               * lookup and update, retrieve the final record
        ...
        const {
          ...
          .select(
            "id, date, role_id, volunteer_id, status"
          )
        ...
        const roleIds = [
          ...new Set(
            entries.map((entry) => entry.role_id)
          ),
        ];
        ...
        const rows: AssignmentRow[] = entries.map(
          (entry) => ({
            entryId: entry.id,
            date: entry.date,
            roleId: entry.role_id,
            roleName:
              roleMap.get(entry.role_id) ??
              "Unknown role",
            status: entry.status,
          })
        );
  ...
  /*
   * ---------------------------------------------------------
   * UNCLAIM AN ASSIGNMENT
   * ---------------------------------------------------------
   */

  async function unclaimAssignment(
    entryId: string
  ) {
    ...
    try {
      /*
       * Verify the assignment still exists and still
      ...
      /*
       * Clear the volunteer but KEEP the schedule row.
       *
       * The published position therefore becomes Open
       * and another volunteer can claim it.
       */
       ...
       * Remove it from My Schedule immediately.
       */
      ...
      <div className="space-y-6">
        <section className="rounded-2xl border bg-white p-6 shadow-sm">
          ...
          <p className="mt-1 text-sm text-gray-600">
            View your upcoming assignments, add them
            to Google Calendar, and step back from a
        ...
        {!currentVolunteer ? (
          <section className="rounded-2xl border bg-white p-6 shadow-sm">
            ...
            <p className="mt-2 text-sm text-gray-600">
              We could not find an active volunteer
              profile matching this account. Please
          ...
          <section className="rounded-2xl border bg-white p-6 shadow-sm">
            ...
            <p className="mt-2 text-sm text-gray-600">
              You do not have any upcoming scheduled
              roles right now.
            </p>
